...
import Swinject
import LocoViper2
import LocoDatatypes


class ___FILEBASENAME___Assembly: Constructible {
    required init() {}
}
...
extension ___FILEBASENAME___Assembly: AssemblyType {
    ...
    func assemble(container: Container) {
        ...
        // feature
        container.register(___FILEBASENAME___FeatureType.self) { resolver in
            return ___FILEBASENAME___Feature(storyboard: resolver.resolve(SwinjectStoryboard.self, name: "___FILEBASENAME___")!)
        }

        // styles
        container.register(___FILEBASENAME___StyleType.self) { resolver in
            return ___FILEBASENAME___Style(colorStyle: resolver.resolve(ColorStyle.self)!, fontStyle:
        ...
        container.register(___FILEBASENAME___Interactor.self) { resolver, presenter in
            ...
            interactor.dataManager = resolver.resolve(___FILEBASENAME___DataManager.self)
        }
